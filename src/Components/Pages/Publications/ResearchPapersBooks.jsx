...
const ResearchPapersBooks = () => {
  const [selectedTab, setSelectedTab] = useState('Research Papers'); // Default to 'Research Papers' tab
        ...
        {sections.map((section) => (
          ...
          >
            {selectedTab === section.id && (
              <TableLayout data={allReportsData.filter(report => report.section === section.id)} />
            )}
          </Box>
        ))}
      </Box>
